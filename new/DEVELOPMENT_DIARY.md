...
### 1.3 Codebase Layer Separation (January 2026)

Following the principle of separation of concerns, the codebase was restructured into distinct layers:

**Core Layer (src/core/)** - WASM Engine:
- `services/` - 5 WASM service wrappers (RDKit, OpenBabel, Vina, PDB, PubChem)
- `workers/` - Background docking worker
- `utils/` - Computational utilities (gridbox, parsers)
- `types/` - TypeScript type definitions

**UI Layer (src/ui/)** - React Components:
- `components/` - 15 React UI components
- `styles/` - 14 CSS files for component styling

This separation enables:
1. Independent development of computation and UI
2. Clearer code navigation and maintenance
3. Potential for core layer reuse in other projects
...
## [2026-01-10] Vina WASM Engine Integration - SUCCESS

### 12.1 Problem Statement

The Vina WASM docking engine was failing with `Worker error: undefined`. Users could not run molecular docking calculations.

### 12.2 Root Causes Identified

1. **Aioli CDN Blocked**: The BioWasm CDN was blocked by COOP/COEP headers (`Cross-Origin-Embedder-Policy: require-corp`)
2. **Worker Type Mismatch**: Module workers (`type: 'module'`) incompatible with Aioli's `importScripts` mechanism
3. **Missing Wrapper**: The high-level `Webina.min.js` wrapper was not in the extracted distribution

### 12.3 Solution Implemented

Created direct integration with Webina using the **low-level Emscripten interface**:

**New File: `src/services/webinaBridge.ts`**
- Dynamic ES module import of `vina.js`
- Output capture via custom `print`/`printErr` callbacks
- Virtual filesystem operations (`FS.writeFile`, `FS.readFile`)
- Execution via `Module.callMain(args)`

**Modified: `src/services/vinaService.ts`**
- Replaced Web Worker approach with main-thread Webina bridge
- Uses `runWebinaVina()` from webinaBridge

### 12.4 Integration Pattern

```typescript
// Load Webina ES module
const moduleImport = await import('/webina/webina/Webina/vina.js');
const WebinaFactory = moduleImport.default;

// Configure with output capture
const Module = await WebinaFactory({
    print: (text) => { stdout += text + '\n'; },
    printErr: (text) => { stderr += text + '\n'; },
    noInitialRun: true,
    locateFile: (path) => '/webina/webina/Webina/' + path
});

// Execute docking
Module.FS.writeFile('/receptor.pdbqt', receptorData);
Module.FS.writeFile('/ligand.pdbqt', ligandData);
Module.callMain(args);
const output = Module.FS.readFile('/output.pdbqt', { encoding: 'utf8' });
```

### 12.5 Browser Test Results

| Test Item | Result |
|-----------|--------|
| Module Loading | ✅ Success |
| FS Available | ✅ object |
| callMain Available | ✅ function |
| Output Generated | ✅ 5793 chars |
| Affinities Extracted | ✅ From stdout |

### 12.6 Files Changed

| File | Change |
|------|--------|
| `src/services/webinaBridge.ts` | NEW - Low-level Webina integration |
| `src/services/vinaService.ts` | MODIFIED - Uses webinaBridge |
| `src/core/services/vinaService.ts` | MODIFIED - Removed unused import |
