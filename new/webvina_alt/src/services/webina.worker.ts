...
async function initializeVina() {
    ...
    vinaModule = await moduleFactory({
        ...
        TOTAL_STACK: 10 * 1024 * 1024, // 10MB

        thisProgram: "vina",

        // CRITICAL: Vina uses recursion (Quicksort, Scoring). 
        // Default stack is small. We need at least 10MB.
        TOTAL_STACK: 10 * 1024 * 1024,

        // CRITICAL: Disable threading to use Main Thread logic (more stable in workers)
        PTHREAD_POOL_SIZE: 0,

        // CRITICAL: Ensure enough Heap
        INITIAL_MEMORY: 536870912, // 512MB
